...
const GroupDetail = () => {
  ...
  const uploadMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ["groupDetail", parsedGroupId] });
  ...
  const handleGroupScheduleSubmit = async (data: {
    ...
    // 일정 수정 모드
    if (editingSchedule) {
      await fetch(`/api/groups/${parsedGroupId}/schedules/${editingSchedule.scheduleId}`, {
        method: "PUT",
  ...
  const leaveMutation = useMutation({
    ...
    onSuccess: () => {
      toast({ title: "그룹 나가기 완료", description: "그룹에서 성공적으로 나갔습니다." });
  ...
  if (!Number.isFinite(parsedGroupId)) {
    return <div className="container py-10 text-center text-muted-foreground">유효하지 않은 그룹입니다.</div>;
  }
  if (isLoading) {
    return <div className="container py-10 text-center text-muted-foreground">그룹 정보를 불러오는 중입니다...</div>;
  }
  if (isError || !group) {
    return <div className="container py-10 text-center text-muted-foreground">그룹 정보를 가져오지 못했습니다.</div>;
  }
    ...
    <div className="container space-y-10 py-10">
      ...
      <section className="flex flex-col gap-6 rounded-xl border bg-card p-6 shadow-sm md:flex-row">
        <div className="flex h-56 w-full items-center justify-center overflow-hidden rounded-lg bg-muted md:w-1/3">
          {group.imageUrl ? (
            ...
            <div className="flex h-full w-full items-center justify-center text-sm text-muted-foreground">대표 이미지가 없습니다.</div>
          )}
        </div>
        ...
        <div className="flex flex-1 flex-col justify-center space-y-4">
          ...
          <p className="text-base text-muted-foreground">{group.description || "등록된 설명이 없습니다."}</p>
          <div className="text-sm font-semibold text-primary">{group.members.length}명 참여중</div>
        </div>
      </section>
      ...
      <section className="grid gap-10 lg:grid-cols-2">
        ...
        <div className="flex flex-col gap-10">
          ...
          {/* 멤버 */}
          <Card>
            <CardHeader><CardTitle>멤버 ({group.members.length})</CardTitle></CardHeader>
            <CardContent>
              {group.members.length ? (
                <ul className="grid gap-3 md:grid-cols-2">
                  {group.members.map((m, i) => (
                    <li key={i} className="rounded-lg border bg-muted/40 px-4 py-3 text-sm font-medium">{m}</li>
                  ))}
                </ul>
              ) : (
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between">
              ...
              <Button size="sm" variant="outline" onClick={() => setPhotoModalOpen(true)}>사진 업로드</Button>
            </CardHeader>
        ...
        <Card className="flex flex-col">
          <CardHeader className="flex flex-row items-center justify-between">
            <div>
              ...
              <p className="text-sm text-muted-foreground">총 {groupSchedules?.length ?? 0}건</p>
            </div>
            ...
            {groupSchedules && groupSchedules.length > 0 ? (
              <div className="max-h-40 overflow-y-auto space-y-3">
                {groupSchedules.map((s) => (
                  <div key={s.scheduleId} className="rounded-lg border bg-muted/20 p-3 space-y-2">
      ...
      <section className="flex justify-center pt-8 border-t">
        <Button variant="destructive" disabled={leaveMutation.isPending} onClick={handleLeaveGroup}>
      ...
      <Dialog open={photoModalOpen} onOpenChange={setPhotoModalOpen}>
        <DialogContent className="max-w-lg p-0">
          <Form {...uploadForm}>
            <form onSubmit={uploadForm.handleSubmit(handlePhotoSubmit)} className="space-y-6 p-6">
              <DialogHeader>
                <DialogTitle>사진 업로드</DialogTitle>
                <DialogDescription>그룹 앨범에 추가할 사진과 정보를 입력해주세요.</DialogDescription>
              </DialogHeader>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl><Input {...field} placeholder="예: 첫 정모" /></FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
                />
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl><Textarea rows={4} {...field} placeholder="사진 설명" /></FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
                />
